                        ...
                        if metrics_list:
                            latest = next(
                                (
                                    ...
                                    if str(metric.get("node_id")) == str(node_id)
                                ),
                                None,
                            )
                            if latest:
    ...
    while True:
        try:
            ...
            if res.status_code == 200:
                ...
                safe_ui(lambda code=res.status_code: append_log(
                    f"âŒ Task poll failed with status: {code}\n"))
                break
        except Exception as e:
            safe_ui(lambda err=e: append_log(f"âŒ Task poll error: {err}\n"))
            break
        time.sleep(1)
...
def start_agent():
    global agent_process, agent_started_by_ui

    python_exe = sys.executable
    agent_script = os.path.join(BASE_DIR, "node_agent.py")
    expected_code_version = str(int(os.path.getmtime(agent_script)))

    # Check if agent is already running
    try:
        res = requests.get(f"{LOCAL_AGENT_URL}/health", timeout=2)
        if res.status_code == 200:
            running_code_version = str(res.json().get("code_version", ""))
            if running_code_version == expected_code_version:
                log("Agent already running")
                agent_started_by_ui = False
                return

            log("Agent is running an older build; restarting it")
            try:
                requests.post(f"{LOCAL_AGENT_URL}/shutdown", timeout=3)
                time.sleep(1)
            except Exception as e:
                log(f"Agent restart request failed: {e}")
    except Exception:
        pass
    ...
    try:
        ...
        while time.time() < deadline:
            if agent_process.poll() is not None:
                log(f"Agent exited early with code {agent_process.returncode}")
                agent_process = None
                agent_started_by_ui = False
                return
        ...
        log("Agent health check timed out")
...
def stop_agent():
    ...
    if agent_process and agent_process.poll() is None:
        ...
        try:
            ...
            log(f"Agent shutdown request failed: {e}")

        try:
            agent_process.wait(timeout=8)
        except Exception:
            try:
                agent_process.terminate()
